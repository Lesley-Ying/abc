...
function setup() {
  ...
  // 允许地图点击，但 UI 层本身不阻挡点击
  canvas.elt.style.pointerEvents = "auto";
  me = new MyPoint();

  // 1. 创建 UI 总容器（关键：它是 fixed，且尺寸随 VisualViewport 变化）
  ...
  progressContainer.id = "progress-ui"; // 这里的 ID 要和 drawUI 里的对应
...
function drawReceivedGroups() {
  ...
  for (let gi = 0; gi < receivedGroups.length; gi++) {
    ...
    strokeWeight(1);
    rectMode(CORNER);
    rect(-PADDING, -PADDING, dispW + PADDING * 2, dispH + PADDING * 2, 6);
