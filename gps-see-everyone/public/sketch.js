...
socket.on("listenforOthers", function (otherPos) {
  otherUsers[otherPos.id] = { lat: otherPos.lat, lon: otherPos.lon }
...
function draw() {
  ...
  if (mapInit) {
    ...
    for (let id in otherUsers) {
      ...
      let pos = myMap.latLngToPixel(user.lat, user.lon);
...
//this function is for the map you see on your phone instead of update position
